...
- ```Programme to Assign```: The ID of the degree programme whose students to assign to modules in this run. The other students will not be assigned modules, or re-assigned if already assigned in a previous run. 
...
If you want to allocate students on different degree programmes while tracking the capacity of modules across all programmes, go back to Step 2, change the ```Programme to Assign``` option, and then press the "Run Allocation" button again. This will ensure that the counts of module capacities are carried over between assignment runs. You can repeat this process any number of times to assign all students from all degree programmes.

If you want to change or re-try the allocations for one of the degree programmes, you will have to restart the app and begin again for all programmes. If you repeatedly press "Run Allocation" to retry, this will reduce the available spaces on the modules, as spaces are shared across allocation runs.
...
This file is similar in structure to ```student_module_preferences.csv```, and is generated by the app. It can be uploaded under the "Prior Allocs" section to allow the previously allocated modules and remaining capacities to be accounted for in a new round of allocation. In general, this will not be necessary if you are repeatedly using Step 2 and Step 3 to assign students to modules one degree programme at a time.
